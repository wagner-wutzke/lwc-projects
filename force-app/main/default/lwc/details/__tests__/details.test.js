...
describe("c-details", () => {
    ...
    it("Delete button is disabled on initial render", async () => {
        ...
        const deleteButton = Array.from(element.shadowRoot.querySelectorAll("lightning-button")).find(
            (b) => b.label === "Delete"
        );
    ...
    it("handleNew sets form mode to edit and clears recordId", async () => {
        ...
        const newButton = Array.from(element.shadowRoot.querySelectorAll("lightning-button")).find(
            (b) => b.label === "New"
        );
        newButton.click();
        await Promise.resolve();
